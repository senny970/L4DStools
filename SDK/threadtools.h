...
typedef uintp (*ThreadFunc_t)( void *pParam );
 ...
 ThreadHandle_t CreateSimpleThread( ThreadFunc_t, void *pParam, unsigned stackSize = 0 );
 bool ReleaseThreadHandle( ThreadHandle_t );


//-----------------------------------------------------------------------------

 void ThreadSleep(unsigned duration = 0);
 ThreadId_t ThreadGetCurrentId();
 ThreadHandle_t ThreadGetCurrentHandle();
 int ThreadGetPriority( ThreadHandle_t hThread = NULL );
...
inline void ThreadPause()
{
 ...
 bool ThreadJoin( ThreadHandle_t, unsigned timeout = TT_INFINITE );
...
// non 32-bit windows and 360 implementation
 int32 ThreadInterlockedIncrement( int32 volatile * ) NOINLINE;
 int32 ThreadInterlockedDecrement( int32 volatile * ) NOINLINE;
 int32 ThreadInterlockedExchange( int32 volatile *, int32 value ) NOINLINE;
 int32 ThreadInterlockedExchangeAdd( int32 volatile *, int32 value ) NOINLINE;
 int32 ThreadInterlockedCompareExchange( int32 volatile *, int32 value, int32 comperand ) NOINLINE;
 bool ThreadInterlockedAssignIf( int32 volatile *, int32 value, int32 comperand ) NOINLINE;
#endif
...
#else
 void *ThreadInterlockedExchangePointer( void * volatile *, void *value ) NOINLINE;
 void *ThreadInterlockedCompareExchangePointer( void * volatile *, void *value, void *comperand ) NOINLINE;
 bool ThreadInterlockedAssignPointerIf( void * volatile *, void *value, void *comperand ) NOINLINE;
#endif
...
inline bool ThreadInterlockedAssignPointerToConstIf( void const * volatile *p, void const *value, void const *comperand )			{ return ThreadInterlockedAssignPointerIf( const_cast < void * volatile * > ( p ), const_cast < void * > ( value ), const_cast < void * > ( comperand ) ); }
 ...
 int64 ThreadInterlockedCompareExchange64( int64 volatile *, int64 value, int64 comperand ) NOINLINE;
 bool ThreadInterlockedAssignIf64( volatile int64 *pDest, int64 value, int64 comperand ) NOINLINE;

 int64 ThreadInterlockedExchange64( int64 volatile *, int64 value ) NOINLINE;
 int64 ThreadInterlockedIncrement64( int64 volatile * ) NOINLINE;
 int64 ThreadInterlockedDecrement64( int64 volatile * ) NOINLINE;
 int64 ThreadInterlockedExchangeAdd64( int64 volatile *, int64 value ) NOINLINE;
...
//-----------------------------------------------------------------------------
 ...
 void ThreadNotifySyncPrepare(void *p);
 void ThreadNotifySyncCancel(void *p);
 void ThreadNotifySyncAcquired(void *p);
 void ThreadNotifySyncReleasing(void *p);
#else
...
#ifdef _OSX
 GenericThreadLocals::CThreadLocalInt<int> g_nThreadID;
#else // _OSX
...
private:
	 bool TryLockInline( const uint32 threadId ) volatile
	{
...
template<class T>  T ReadVolatileMemory( T const *pPtr )
{
